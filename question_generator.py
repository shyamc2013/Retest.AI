...
Aim for a useful mix of `mcq_single`, `mcq_multiple`, `true_false`, and `fill_blank`, based on what the material supports. Do not repeat questions from a prior attempt. `mcq_single` has exactly one correct option: provide option text in `options` and make `correct_answer` that option string. `mcq_multiple` has two or more correct options: provide option text in `options` and make `correct_answer` a list containing every correct option string. For true_false, use `correct_answer` as the JSON string `"True"` or `"False"` (never a JSON boolean) and leave options empty. For fill_blank, leave options empty and make the answer concise. IDs must be unique integers from 1 through {QUESTION_COUNT}. Never use literal double-quote characters inside `question_text` or `options`; rephrase or use single quotes instead, since embedded double quotes tend to break JSON formatting.
...
def generate_questions(document_text: str) -> list[Question]:
    """Generate a complete test, retrying a few times for transient provider failures."""
    errors: list[str] = []
    for _ in range(3):
        try:
    ...
    raise RuntimeError("Question generation failed after three Groq attempts. " + " | ".join(errors))
